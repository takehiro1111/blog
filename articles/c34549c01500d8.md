...
locals {
    ...
    ami = "ami-0dafcef159a1fc745" // AmazonLinux2 の AMI を変数で 定義
}
```
...
resource "aws_instance" "test" {
 ...
 // EC2 インスタンスの中で httpd が起動してドキュメントルートに index.html ファイルが置かれるよう設定
 ...
 //EC2 インスタンスにデフォルトでアタッチされる EBS ボリュームの設定 
    root_block_device {
        volume_type = "gp3" 
        volume_size = 8 
...
```
tfenv list // 使用可能な terraform のバージョンを確認
tfenv install 1.6.5 // 使用予定の terraform バージョンをインストール
tfenv use 1.6.5 // インストールしたバージョンを使用できるよう設定
