...
:::message
- S3 Intelligent-Tiering は、オートティアリングで対象となるオブジェクトの最小サイズが 128 KB となっています。128KB未満のオブジェクトはモニタリングされず、常に高頻度アクセスティア料金で課金され、モニタリング料金やオートメーション料金は発生しません。
:::

&nbsp;
## 3.導入にあたって気にしたい点
### 3-1.対象のファイルサイズ
- ファイルのサイズが128KB未満だとアクセスパターンのモニタリング対象に入らず、高頻度階層のストレージに残る。
